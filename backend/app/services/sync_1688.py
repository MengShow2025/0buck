...
class Sync1688Service:
    ...
    async def trigger_sourcing(self, order_id: int, line_items: List[Dict[str, Any]]):
        ...
        for item in line_items:
            ...
            # 1. Try SKU (format: 1688-ID)
            product_id_1688 = None
            if sku and sku.startswith("1688-"):
                product_id_1688 = sku.replace("1688-", "")
            
            # 2. Try Metafields if SKU fails
            if not product_id_1688 and product_id:
                try:
                    sp = shopify.Product.find(product_id)
                    # Fetch metafields
                    mfs = sp.metafields()
                    for mf in mfs:
                        if mf.namespace == "0buck_sync" and mf.key == "source_1688_id":
                            product_id_1688 = mf.value
                            break
                except Exception as e:
                    print(f"  Error fetching metafields for product {product_id}: {e}")

            if not product_id_1688:
                print(f"  Skipping item {item.get('title')} - No 1688 ID found in SKU/Metafields")
                continue
